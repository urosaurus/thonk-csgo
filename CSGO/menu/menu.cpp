...
namespace menu
{
	...
	void insert_menu_items()
	{
		...
		if (vars::vis::vis_tab) {
			...
			if (vars::vis::vis_color_tab) {
				...
				if (!vars::vis::vis_ambient_tab)
					i = add_menu_entry(i, "[+] Ambient Lighting", &vars::vis::vis_ambient_tab);
				if (vars::vis::vis_ambient_tab) {
		...
		if (vars::hvh::hvh_tab) {
			...
			i = add_menu_entry(i, "- Fake Walk (LSHIFT)", &vars::hvh::hvh_fake_walk);
		...
		if (vars::misc::misc_tab) {
			...
			i = add_menu_entry(i, "- Animated Clantag", &vars::misc::misc_clan_tag_changer);
	...
	void draw_menu(unsigned long font)
	{
		...
		// title
		d::draw_string(vars::menu::menu_x + 5, vars::menu::menu_y - 14, color(120, 120, 120), font, " csgohack:");

		// arrow
		d::draw_string(vars::menu::menu_x - 10, vars::menu::menu_y + 2 + (vars::menu::font_size * vars::menu::current_pos) + 5, color(120, 120, 120), font, ">");

		// times
		for (int i = 0; i < vars::menu::total_items; ++i) {

			if (!strncmp(items[i].name.c_str(), "[+]", 3) || !strncmp(items[i].name.c_str(), "[-]", 3)) {
				d::draw_string(vars::menu::menu_x, vars::menu::menu_y + 2 + (vars::menu::font_size * i) + 4, color(255, 255, 255, 255), font, items[i].name.c_str());
			} else {

				switch (items[i].type) {
				case 0:
					d::draw_string(vars::menu::menu_x, vars::menu::menu_y + 2 + (vars::menu::font_size * i) + 4, color(255, 255, 255, 255), font, items[i].name.c_str());
					d::draw_string(vars::menu::menu_x + 150, vars::menu::menu_y + 2 + (vars::menu::font_size * i) + 4, color(255, 255, 255, 255), font, "%s", (*items[i].m_bool) ? "On" : "Off");
					break;
				case 1:
					d::draw_string(vars::menu::menu_x, vars::menu::menu_y + 2 + (vars::menu::font_size * i) + 4, color(255, 255, 255, 255), font, items[i].name.c_str());
					d::draw_string(vars::menu::menu_x + 150, vars::menu::menu_y + 2 + (vars::menu::font_size * i) + 4, color(255, 255, 255, 255), font, "%i", *items[i].m_int);
					break;
				case 2:
					d::draw_string(vars::menu::menu_x, vars::menu::menu_y + 2 + (vars::menu::font_size * i) + 4, color(255, 255, 255, 255), font, items[i].name.c_str());
					d::draw_string(vars::menu::menu_x + 150, vars::menu::menu_y + 2 + (vars::menu::font_size * i) + 4, color(255, 255, 255, 255), font, "%.2f", *items[i].m_float);
					break;
				case 3:
					d::draw_string(vars::menu::menu_x, vars::menu::menu_y + 2 + (vars::menu::font_size * i) + 4, color(255, 255, 255, 255), font, items[i].name.c_str());
					break;
				...
				if (!strcmp(items[i].name.c_str(), "- Mode")) {
					d::draw_string(vars::menu::menu_x + 150, vars::menu::menu_y + 2 + (vars::menu::font_size * i) + 4, color(255, 255, 255, 255), font, "%s", esp_options[(int)items[i].m_int[0]]);
				}

				if (!strcmp(items[i].name.c_str(), "- Hitbox")) {
					d::draw_string(vars::menu::menu_x + 150, vars::menu::menu_y + 2 + (vars::menu::font_size * i) + 4, color(255, 255, 255, 255), font, "%s", hitbox_options[(int)items[i].m_int[0]]);
				}

				if (!strcmp(items[i].name.c_str(), "- Box")) {
					d::draw_string(vars::menu::menu_x + 150, vars::menu::menu_y + 2 + (vars::menu::font_size * i) + 4, color(255, 255, 255, 255), font, "%s", box_options[(int)items[i].m_int[0]]);
				}

				if (!strcmp(items[i].name.c_str(), "- Health")) {
					d::draw_string(vars::menu::menu_x + 150, vars::menu::menu_y + 2 + (vars::menu::font_size * i) + 4, color(255, 255, 255, 255), font, "%s", health_options[(int)items[i].m_int[0]]);
				}

				if (!strcmp(items[i].name.c_str(), "- Armor")) {
					d::draw_string(vars::menu::menu_x + 150, vars::menu::menu_y + 2 + (vars::menu::font_size * i) + 4, color(255, 255, 255, 255), font, "%s", armor_options[(int)items[i].m_int[0]]);
				}

				if (!strcmp(items[i].name.c_str(), "- Thirdperson (H)")) {
					d::draw_string(vars::menu::menu_x + 150, vars::menu::menu_y + 2 + (vars::menu::font_size * i) + 4, color(255, 255, 255, 255), font, "%s", tp_options[(int)items[i].m_int[0]]);
				}

				if (!strcmp(items[i].name.c_str(), "- Pitch")) {
					d::draw_string(vars::menu::menu_x + 150, vars::menu::menu_y + 2 + (vars::menu::font_size * i) + 4, color(255, 255, 255, 255), font, "%s", aa_pitch[(int)items[i].m_int[0]]);
				}
